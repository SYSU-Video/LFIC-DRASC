...
##original padding
    ...
    Rec = img_window_reverse(Rec, p, height, width)  # inverse(N,C,Hpatch,Wpatch)->（1,C,H,W)
    Rec = F.pad(
...
def eval_model(model, filepaths, entropy_estimation=False, half=False, outputpath='Recon', patch=832):
...
def main(argv):
    ...
    model_cls = BaseModel(N=48, M=48, angRes=13, n_blocks=1)
