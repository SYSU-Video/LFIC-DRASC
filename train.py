...
def main(argv):
    ...
    net = BaseModel(N=args.N, M=args.N, angRes=args.angRes, n_blocks=args.n_blocks)
    net = net.to(device)
    ...
    if args.checkpoint:  # load from previous checkpoint
        ...
        checkpoint["state_dict"] = {f"module.{k}": v for k, v in checkpoint["state_dict"].items()} # 注意这几行
